...
- Use teal/blue (#1b7ea8) for Yang (generative intake) paths.
- Use coral/warm red (#d1615a) for Yin (stabilizing return) paths.
...
- The three layer labels ("Computational Reality", "Functional Interpretation", "Harmony Parallel") appear as LEFT MARGIN LABELS ONLY, vertically stacked, framing the single image.
...
- Context window block:
  - Horizontal band or container behind/around the main stack labeled: "Context Window".
  - This is where the Micro-Buoyancy Manager will attach.
- Transformer layers:
  - At least two stacked groups, each group containing:
    - An "Attention" sub-block (upper) labeled: "Multi-Head Attention".
    - An "MLP/FFN" sub-block (lower) labeled: "MLP / Feed-Forward".
  - The layers may be abstracted as repeated units with a bracket or label: "Transformer Layers".
- Output block:
...
- Within a representative transformer layer:
  - Solid grey arrow: "Attention" → "MLP / FFN" → next layer.
...
- A prominent coral return loop starting at the Output:
  ...
  - From the Output-Coherence Sensor → back into:
    - The Attention Rebalancer (attached to the Attention sub-blocks).
...
- Shape & label:
  - Rectangular block labeled:
    - Main label: "Output-Coherence Sensor"
    - Subtext: "Monitors coherence, contradictions, drift".
- Connections:
  - Solid coral arrow from "Output Logits / Tokens" → "Output-Coherence Sensor".
...
- Shape & label:
  - Rectangular or bracket-style block labeled:
    - "Attention Rebalancer".
    - Small subtext: "Reweights attention based on coherence signal".
- Connections:
  - Coral input arrow from "Output-Coherence Sensor" into "Attention Rebalancer".
  - The Attention Rebalancer then injects adjustments into the Attention sub-blocks (short arrows or overlay lines).
...
- Shape & label:
  - Rectangular block spanning the main forward path, labeled:
    - "Phase-Flip Controller".
    - Small subtext: "Exploration ↔ Consolidation mode switch".
- Behavior:
  - Teal arrows (Yang) pass through it from Transformer Layers → Output.
...
- Shape & label:
  - Rectangular block labeled:
    ...
    - Small subtext: "Context refresh / oscillatory stabilization".
- Connections:
  - Coral dotted arrow from "Output-Coherence Sensor" (optional).
  - Grey arrows to/from the Context Window region to indicate periodic refreshing and re-weighting.
...
- Coral dotted or dashed arrows for secondary feedback:
  - Output-Coherence Sensor → Micro-Buoyancy Manager.
...
- Size hierarchy:
  ...
  - Mechanism subtexts: smaller but legible.
  - Harmony parallels: small, italic.
...
- One high-resolution SVG (3000×2000 px) following this specification.
- One high-resolution PNG (3000 px longest side) exported from the SVG.
- One low-resolution preview PNG (800 px longest side) for quick review.
